...
def generate_moving_mnist(shape=(64,64), seq_len=30, seqs=10000, num_sz=28, nums_per_image=2):
    ...
    dataset = np.empty((seqs, seq_len, 1, width, height), dtype=np.float32)
    for seq_idx in xrange(seqs):
        # randomly generate direc/speed/position, calculate velocity vector
        direcs = np.pi * (np.random.rand(nums_per_image)*2 - 1)
        speeds = np.random.randint(1, size=nums_per_image)+5
        ...
        for frame_idx in xrange(seq_len):
            ...
            # print( "canvas", np.min( canvas ), np.max( canvas ) )
            dataset[seq_idx, frame_idx] = ( canvas.clip(0.0,1.0) )
            # print( "canvas", np.min( dataset[frame_idx, seq_idx] ), np.max( dataset[frame_idx, seq_idx] ) )
