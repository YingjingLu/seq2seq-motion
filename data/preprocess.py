...
def generate_moving_mnist(shape=(64,64), seq_len=50, seqs=10000, num_sz=28, nums_per_image=2):
    mnist = load_dataset()
    width, height = shape
    ...
    for seq_idx in xrange(seqs):
        ...
        speeds = np.random.randint(1, size=nums_per_image)+2
        ...
        for frame_idx in xrange(seq_len):
            ...
            dataset[frame_idx, seq_idx, :, :] = (np.squeeze( canvas ) ).astype(np.float32).clip(0.0,1.0)
    return dataset
